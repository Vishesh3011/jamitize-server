...
func CreateUserController(app application.Application, _ http.ResponseWriter, request *http.Request) (*models.UserResponse, errors.AppError) {
	...
	refreshToken, err := utils.GenerateJWTRefresh(user.ID, user.Email, app.Config().JwtConfig().Secret())
...
//
//	user, appError := service.NewService(ctx, app.Logger(), app.DB()).User().RefreshUserToken(email, token)
//	if appError != nil {
