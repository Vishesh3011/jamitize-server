...
func NewUserService(ctx context.Context, logger *slog.Logger, db *mongo.Database, key string) UserService {
	return &userService{
		ctx:    ctx,
		logger: logger,
		db:     db,
...
func (u userService) FindUserByEmail(email string) (*models.User, errors.AppError) {
...
func (u userService) LoginUser(request *models.UserLoginRequest) (*models.User, errors.AppError) {
	...
	refreshToken, err := utils.GenerateJWTRefresh(user.ID.Hex(), user.Email, u.key)
	if err != nil {
