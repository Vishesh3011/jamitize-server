...
func (u userService) RefreshUserToken(email, token string) (*models.User, errors.AppError) {
	user, appError := repository.NewRepository(u.ctx, u.logger, u.db).User().FindUserByEmail(email)
	if appError != nil {
		u.logger.Error("Failed to find user by email for token refresh", "error", appError)
		return nil, appError
	}
	...
	if appError != nil {
		u.logger.Error("Failed to update user refresh token", "error", appError)
		return nil, appError
	}
	return user, nil
}
